...
function createFile(fileName, htmlContent, baseFolder) {
  console.log(
    `Creating PDF file: ${fileName}.pdf at ${path.resolve(
      baseFolder,
      `${fileName}.pdf`
    )}`
  );
  return new Promise((resolve, reject) => {
    pdf
      .create(htmlContent, { height: "288px", width: "600px", timeout: 900000 })
...
function createBaseDirectory(baseFolder) {
  if (!fs.existsSync(path.resolve(baseFolder)))
    return fs.mkdirSync(baseFolder, { recursive: true });
}
